...
{
    /// <summary>
    /// Implementation based on local file system of an <see cref="IRepository"/>.
    /// </summary>
    public class FileRepository : IRepository
    {
        #region Fields

        private readonly string repositoryPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "//ObjectStore//";
        private readonly string fileExtension = ".data";
        ...
        /// </summary>
        /// <exception cref="RepositoryException">Is thrown in all cases where the repository could not be created.</exception>
        /// <remarks>
        /// All found files are loaded as <see cref="BusinessObject"/>. The conversion to the specific type must be done outside of this class.
        /// </remarks>
        public FileRepository()
        {
            try
            {
                ...
                foreach (FileInfo wikiFile in wikiFiles)
                {
                    ...
                    if (this.TryLoadFileAs<BusinessObject>(fileName))
                    {
                        continue;
                    }
                    else
        ...
        public void Delete(string id)
        {
            ...
            {
                throw new RepositoryException("No wiki object with id: " + id + " found.");
            }
        }
        ...
        public string Store<T>(T businessObjectToStore) where T : BusinessObject
        {
            ...
            {
                throw new RepositoryException("The given business object could not be serialized.", ioe);
            }
        ...
        public IEnumerable<string> GetIdentifiers()
        {
            ...
            {
                throw new RepositoryException("No identifiers at all.");
            }
            else
            {
        ...
        /// <summary>
        /// Tries the load file as type T.
        /// </summary>
        /// <typeparam name="T">The desired (or assumed) type to load.</typeparam>
        /// <param name="fileName">Name of the file where the data shoud be stored.</param>
        /// <returns>true if the file could be loaded as T; false otherwise.</returns>
